...
export default function ReaderLoginPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError('');
    try {
      const data = await apiFetch<ReaderAuthResponse>('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ email, password }),
      });
      setAuth(data.user, data.accessToken, data.refreshToken);
      navigate(pathFor('/account', code));
    } catch (err) {
  ...
  return (
    <>
      <SeoHead title={t('auth.signIn')} noIndex />
      <AuthShell title={t('auth.welcomeBack')}>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="flex justify-between text-sm">
            <Link to={pathFor('/register', code)} className="text-primary-600">
              {t('auth.createAccount')}
            </Link>
            <Link to={pathFor('/forgot-password', code)} className="text-gray-500">
