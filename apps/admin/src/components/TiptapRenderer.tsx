...
function renderNode(node: TiptapNode, index: number): React.ReactNode {
  switch (node.type) {
    ...
    case 'imageGallery': {
      ...
      const layout = getGalleryLayout((node.attrs?.layout as string) || '');
      if (!layout || !images.length) return null;
      return (
        <div key={index} className={`my-6 ${layout.containerClass}`}>
          {images.map((img, i) => (
            <img key={i} src={img.src} alt={img.alt || ''} className={`w-full rounded-lg object-cover ${layout.itemClasses[i] ?? 'aspect-square'}`} />
          ))}
        </div>
      );
    }
